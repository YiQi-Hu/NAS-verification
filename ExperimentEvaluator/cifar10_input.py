...
# data_dir路径
# batch_size一个组的大小
def inputs(eval_data, data_dir, batch_size,NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN=0):
    if not eval_data:
        file_n=int(np.ceil(NUM_EXAMPLES_PER_EPOCH_FOR_EVAL/10000))+1
        filenames = [os.path.join(data_dir, 'data_batch_%d.bin' % i)
                     for i in xrange(1, file_n)]
