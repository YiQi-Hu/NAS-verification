...
class Optimizer:
    ...
    def follow_constraint(self, x):
        # print(x)
        # print("dim:")
        # print(self.__dimension.get_regions())
        # print("cons:")
        # print(self.__constraint)
        for f_i in range(len(self.__constraint)):
            # print()
            prob_sum = 0.0
            for f_j in range(len(self.__constraint[f_i])):
                prob_sum += x[self.__constraint[f_i][f_j]]

            # print(f_i," ", prob_sum)
            # print(x)
            # print(self.__constraint[f_i])
            if prob_sum > 1.0:
                # print(x[self.__constraint[f_i][0] : self.__constraint[f_i][len(self.__constraint[f_i])]])
                # print(f_i," ",prob_sum)
                # print(self.__constraint[f_i])
                return False
    ...
    def sample(self):
        ...
        if self.__sample_count < self.__sample_size + self.__positive_num:
            while True:
                ...
                ins = self.random_instance(self.__dimension, self.__region, self.__label)
                if self.instance_in_list(ins, self.__pop, self.__sample_count) is False:
                    if self.follow_constraint(ins.get_features()) is True:
                        break
        # optimization step
        else:
            while True:
