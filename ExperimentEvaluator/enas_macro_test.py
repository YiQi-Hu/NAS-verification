...
class Evaluator:
    ...
    def _makeconv(self, inputs, hplist, node,sep=False):
        ...
        with tf.variable_scope('conv' + str(node) + 'block' + str(self.blocks)) as scope:
            ...
            if not sep:
                ...
                w_depth=tf.get_variable('depth_weights', shape=[hplist[2], hplist[2], inputdim, 1],
                                     initializer=tf.truncated_normal_initializer(stddev=0.1))
                w_point=tf.get_variable('point_weights', shape=[1, 1, inputdim, hplist[1]],
                                     initializer=tf.truncated_normal_initializer(stddev=0.1))
                conv=tf.nn.separable_conv2d(inputs,w_depth,w_point,strides=[1,1,1,1],padding='SAME')
    ...
    def _makepool(self, inputs, hplist):
        ...
        if hplist[1] == 'avg':
            ...
            output = tf.reduce_mean(inputs, [1, 2], keep_dims=True)
    ...
    def _inference(self, images, graph_part, cellist):  # ,regularizer):
        ...
        for node in range(nodelen):
            ...
            elif cellist[node][0] == 'sep':
                layer = self._makeconv(inputs[node], cellist[node], node,True)
            ...
            for j in graph_part[node]:
                if getinput[j]:  # if this cell already got inputs from other cells precedes it
                    ...
                    inputdim = inputs[j].shape[3]
                    kernel = tf.get_variable('1x1weights'+str(node)+str(j), shape=[1, 1, inputdim, inputdim],
                                             initializer=tf.truncated_normal_initializer(stddev=0.1))
                    inputs[j]=tf.nn.conv2d(inputs[j],kernel,[1,1,1,1],padding='SAME')
                    inputs[j]=tf.nn.relu(batch_norm(inputs[j]))
                else:
                    inputs[j] = layer
                    getinput[j] = True
...
if __name__ == '__main__':
    ...
    cell=[('sep',36,5,'relu'),('conv',36,5,'relu'),('conv',36,5,'relu'),('sep',36,5,'relu'),('pooling','max',2),('sep',36,3,'relu'),('conv',36,5,'relu'),('sep',36,3,'relu'),('sep',36,5,'relu'),('pooling','max',2),('conv',36,5,'relu'),('sep',36,5,'relu'),('conv',36,3,'relu'),('sep',36,5,'relu'),('pooling','global')]
    graph=[[1,3,4,6,8,9,13],[2,11,12],[3,7,8,11,12],[4,6,7,8,9,11,13,14],[5],[6,7,8,9,12,13],[7,8,10],[8,9,11,13,14],[9],[10],[11,12],[12,13,14],[13],[14],[]]
